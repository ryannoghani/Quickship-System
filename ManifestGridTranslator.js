const Container = require('./Container');   //We're including the container class

class ManifestGridTranslator
{
    ...
    ConvertManifestToGrid(manifestFileName)
    {
        ...
        for (var i = 9; i >= 2; --i) //This loops through various parts of the string to extract the weight and name of each container
        {
            for (var j = 0; j < 12; ++j)
            {
                index += 9;
                ...
                while (manifestString.substring(indexOfNewline,indexOfNewline+1) != '\n' && indexOfNewline < manifestString.length) //Finds index of newline character, which indicates when the name has ended
                {
                    indexOfNewline++;
                }
    ...
    ConvertGridToManifest(manifestFileName, grid)    //UPDATES MANIFEST BASED ON CONTENTS OF GRID
    {
        var line = '';
        var row;
        var column;
        for (let i = 9; i >= 2; i--) {        
            for (let j = 0; j < 12; j++) {
                ...
                {
                    for (let k = 1; k <= 5 - (Math.floor(Math.log10(grid[i][j].weight)) + 1); ++k)
                    {
                        line += '0';
                    }
                }
...
module.exports = ManifestGridTranslator;